<template>
  <div class="space-y-8">
    ...
    <div class="text-center">
      ...
      <p v-if="tag?.description" class="text-lg text-gray-600 mb-4">
        {{ tag.description }}
      </p>
      <p class="text-sm text-gray-500">
        {{ blogStore.totalBlogs }} post{{ blogStore.totalBlogs !== 1 ? 's' : '' }} tagged with #{{ tag?.name }}
      </p>
    </div>
      ...
      >
        ...
        <div class="p-6">
          ...
          <div v-if="blog.tags && blog.tags.length" class="mt-4 flex flex-wrap gap-1">
            <span
              v-for="blogTag in blog.tags.slice(0, 3)"
              :key="blogTag.id"
              class="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-gray-100 text-gray-800"
              :class="{ 'bg-primary-100 text-primary-800': blogTag.name === tag?.name }"
            >
              #{{ blogTag.name }}
            </span>
            <span
...
const loadTagPosts = async () => {
  try {
    ...
    const allTags = await blogStore.fetchTags()
    tag.value = allTags.find(t => t.slug === tagSlug)
    ...
    await blogStore.fetchBlogsByTag(tagSlug, {
      page: blogStore.currentPage,
      per_page: blogStore.pagination.per_page
    })
  } catch (error) {

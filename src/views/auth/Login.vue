      ...
      <form class="mt-8 space-y-6" @submit.prevent="handleLogin">
        ...
        <div>
          <button
            ...
            :disabled="auth.isLoading"
          >
            <span v-if="auth.isLoading" class="absolute left-0 inset-y-0 flex items-center pl-3">
...
const validateForm = () => {
  ...
  else if (!emailRegex.test(form.value.email)) errors.value.email = 'Invalid email address';
...
const handleLogin = async () => {
  ...
  try {
    ...
    globalError.value = e?.error?.message || e?.message || 'Login failed. Please try again.';
  }
};
</script>
...
<style scoped>
.input{
  @apply w-full px-3 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-primary-500;
}
